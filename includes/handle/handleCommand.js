...
//// admin -pa /////
    ...
    if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) {
      if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) return api.sendMessage('[ مـــــود ] - فقط المطورة إلين من يمكنها تفعيله', threadID, messageID)
    }
    ...
    if (!command) {
      ...
      if (checker.bestMatch.rating >= 1.0) command = client.commands.get(checker.bestMatch.target);
    }
