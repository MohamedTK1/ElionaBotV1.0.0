...
(async function () {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`╰┈➤ BODY | سيتم الموافقة من قبل المسؤول🕊📿✨🥛`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(` ╰┈➤ BODY | لم تتم الموافقة على الجروب من قبل المسؤول بعد،، \n لطلب الموافقة اكتب ${prefix}request\n\n\n\╰┈➤ INF | للمعلومات تواصل مع ادمن البوت ⌯ m.me/ElinYeager726 👽👋🏻`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
